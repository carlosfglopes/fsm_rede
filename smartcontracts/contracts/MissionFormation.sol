...
/// @title MissionFormation — decentralized UAV formation-keeping FSM
/// @notice UAVs report position; the contract checks each one's distance to the
///         swarm centroid against configured thresholds; peers vote on
///         violations, degrading or reconfiguring the mission, and vote again
...
contract MissionFormation {
    ...
    MissionState private stateBeforeReconfig;
    ...
    modifier inOperationalState() {
        require(
            ...
            missionState == MissionState.RECONFIGURING_FORMATION ||
            missionState == MissionState.DEGRADED,
            "Mission not operational"
        );
        _;
    }
    ...
    function registerUAV(address _uav, int256 _x, int256 _y)
        external onlyAuthority inMissionState(MissionState.SETUP)
    {
    ...
    function changeFormation(uint256 _formationId, uint256 _dMinSq, uint256 _dMaxSq, uint256 _rMaxSq)
        external onlyAuthority inOperationalState
    {
        require(_dMinSq < _dMaxSq, "dMinSq must be < dMaxSq");

        pendingFormation    = FormationParams(_formationId, _dMinSq, _dMaxSq, _rMaxSq);
        stateBeforeReconfig = missionState;
    ...
    function finalizeFormationChange()
        external onlyAuthority inMissionState(MissionState.RECONFIGURING_FORMATION)
    {
    ...
    function checkLateUAVs() external onlyAuthority inOperationalState {
        for (uint256 i = 0; i < uavList.length; i++) {
            address u = uavList[i];
    ...
    function deactivateUAV(address _uav) external onlyAuthority {
    ...
    function completeMission() external onlyAuthority {
        require(
            missionState == MissionState.ACTIVE ||
            missionState == MissionState.DEGRADED,
    ...
    function abortMission(string calldata reason) external onlyAuthority {
        require(
            missionState != MissionState.COMPLETED &&
   ...
   function updatePosition(int256 _x, int256 _y)
        external onlyRegisteredActiveUAV inOperationalState
    {
        ...
        if (missionState == MissionState.RECONFIGURING_FORMATION &&
            block.timestamp >= transitionEnd) {
            _applyFormationChange();
        }
    ...
    {
        bool    transition = missionState == MissionState.RECONFIGURING_FORMATION;
        uint256 timeLeft   = (transition && block.timestamp < transitionEnd)
            ? transitionEnd - block.timestamp
            : 0;

        return (missionState, currentFormation.formationId, centroidX, centroidY,
    ...
    function _checkSwarmHealth() internal {
        if (missionState == MissionState.RECONFIGURING_FORMATION) return;

        uint256 nonOK = getNonOKCount();

        if (nonOK >= degradedThreshold && missionState == MissionState.ACTIVE) {
            missionState = MissionState.DEGRADED;
            emit MissionStateChanged(missionState);
        } else if (nonOK < degradedThreshold && missionState == MissionState.DEGRADED) {
            missionState = MissionState.ACTIVE;
            emit MissionStateChanged(missionState);
        }
    }
    ...
    function _applyFormationChange() internal {
        ...
        missionState = stateBeforeReconfig;
