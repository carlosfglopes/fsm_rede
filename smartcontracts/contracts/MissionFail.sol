...
/// @title MissionFail — decentralized UAV failure-detection and recovery FSM
/// @notice UAVs heartbeat; the authority opens an incident on timeout or
///         misbehavior; peers vote by quorum to confirm or reject the suspect;
///         the mission reconfigures, degrades, or aborts based on the outcome.
contract MissionFail {
    ...
    // EXTERNAL FUNCTIONS — AUTHORITY (SETUP)


    function registerUAV(address _uav, uint256 _capacityMax)
        external
        onlyAuthority
        inMissionState(MissionState.SETUP)
    {
    ...
    // EXTERNAL FUNCTIONS — AUTHORITY (RUNTIME)
    ...
    function finalizeIncident()
        external
        onlyAuthority
        inMissionState(MissionState.UNDER_CONFIRMATION)
    {
    ...
    function triggerReconfiguration()
        external
        onlyAuthority
        inMissionState(MissionState.RECONFIGURING)
    {
        require(
            uavs[suspectUav].state == UAVState.CONFIRMED_FAILED ||
            uavs[suspectUav].state == UAVState.CONFIRMED_BYZANTINE,
    ...
    function setFormation(FormationMode _formationMode) external onlyAuthority {
        formationMode = _formationMode;
    ...
    function recoverToActiveReconfigured()
        external
        onlyAuthority
        inMissionState(MissionState.DEGRADED)
    {
    ...
    function completeTask(uint256 taskId) external onlyAuthority {
        ...
        if (assignedTo != address(0) && uavs[assignedTo].registered) {
            if (uavs[assignedTo].loadCurrent > 0) {
                uavs[assignedTo].loadCurrent -= 1;
            }
        }
    ...
    function completeMission() external onlyAuthority inOperationalState {
    ...
    function getTaskSummary(uint256 taskId)
        ...
        returns (
            bool   active,
            address assignedTo,
            uint256 assigneeLoad,
            uint256 assigneeCapacity
        )
    {
        ...
        )
    {
        uint256 uavCount;
        for (uint256 i = 0; i < uavList.length; i++) {
            if (uavs[uavList[i]].state == UAVState.ACTIVE) uavCount++;
        }
        uint256 taskCount;
        for (uint256 i = 0; i < activeTaskIds.length; i++) {
            if (tasks[activeTaskIds[i]].active) taskCount++;
        }
        return (
            missionState,
            formationMode,
