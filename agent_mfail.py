#!/usr/bin/env python3
"""
Autonomous UAV agent — MissionFail. Runs independently on each Raspberry Pi,
connects to its local Besu node, and reacts to the on-chain mission state
(sends heartbeats, votes on incidents).

Usage:
    UAV_ID=UAV1 PRIVATE_KEY=0x... CONTRACT_ADDRESS=0x... python3 agent_mfail.py
...
Environment variables:
    ...
    UAV_ID           (default: UAV1)
    CAPACITY         (maximum UAV capacity, default: 2)
"""
...
CAPACITY         = int(os.getenv("CAPACITY", "2"))
POLL_INTERVAL    = 2
...
def main():
    ...
    log(f"Connected to {RPC_URL}")
    log(f"Address : {account.address}")
    log(f"Capacity: {CAPACITY}")
    log(f"Balance : {w3.from_wei(w3.eth.get_balance(account.address), 'ether')} ETH")
    ...
    while True:
        ...
        try:
            ...
            if state in OPERATIONAL_STATES:
                ...
                log("Mission ABORTED. Agent terminating.")
                break
